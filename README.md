...
- **任务编排控制**: 多轮对话的上下文流转与工作流管理
- **知识范式迁移**: 跨领域/跨时代的能力适配与表达框架转换
...
├── task/
│   └── orchestration/       # 任务编排提示词（工作流控制与上下文流转）
├── style/
│   ├── eras/               # 知识范式迁移提示词（跨领域表达框架转换）
│   └── lighting/           # 光影效果提示词（视觉-文本映射，仅用于多模态理解）
...
### 任务编排控制（Task Orchestration）
设计面向Agent的多轮交互上下文流转逻辑，实现跨模态信息追踪、状态机管理与目标导向的任务达成控制。通过提示词工程实现对智能体行为序列的精确调控。

### 知识范式迁移（Knowledge Paradigm Migration）
迁移不同历史时期的表达范式与知识框架，使智能体能够跨越领域边界进行自适应响应与跨语境任务执行。实现从领域A到领域B的知识框架映射与能力复用。
